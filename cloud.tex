...
The algorithm consists of two parts -- \textbf{fitJobMR}/\textbf{fitJobSpark} that finds the optimal parameters on a given instance type for MR/Spark Job and $optimizeCost$ (Algorithm~\ref{cost_optimize}) that searches across instance types to determine the best instance type to be used. Due to lack of space, we list only algorithm \textbf{fitJobSpark} (Algorithm~\ref{jobfitspark}) for SparkSQL jobs. \textbf{fitJobMR} for Hive on MR is similar in nature and is listed in~\cite{fullpaper}.
...
\STATE $\mathcal{P}_{new}$[spark.executor.memory] $\gets$ $\mathcal{I}$[nodeMemory] $\times$ $\mathcal{G}$[nodeMemFrac]
\STATE newMemPerCore $\gets$ ($\mathcal{I}$[nodeMemory] $\times$ $\mathcal{G}$[nodeMemFrac]) $/$ $\mathcal{I}$[vCpuPerNode]
