...
As long as tasks do not spill, the total work done in terms of IO, CPU and network traffic is independent of the available memory and the parallelism factor of the tasks. This leads to the second insight:
\begin{insight}
...
We studied the relationship between the container memory (mapper memory, reducer memory or spark executor memory) and the amount of data processed by the container, i.e. the partition size (determined by splitsize for mappers, bytes per reducer for MR reducers and shuffle partitions for Spark reduce tasks). We performed two sets of experiments on the Spark engine - in the first set, the partition size is kept constant and the container memory is varied and in the second set, the partition size is varied while holding the container memory constant. The experiments were run for the TPC DS dataset (scale 1000) and queries.  The average data read by each query was of around 20 to 40 GB. The experiments were run on a 4 node cluster on AWS with machine type r3.xlarge.
