...
\begin{tabular}{ |l|p {5.3 cm}| } 
 ...
 ioSort & Buffer memory in MB to be used for sorting (\textit{mapreduce.task.io.sort.mb}) \\
...
\begin{table}[h]
	\begin{center}
\begin{tabular}{ |l|p {4.8 cm}|p {1 cm} | } 
 \hline
 ...
 reducerFrac & Reducer memory fraction used as buffer & 0.4 \\ 
 minMapTime & Minimum time that a mapper should take & 60s \\
 minRedTime & Minimum time that a reducer should take & 60s \\
 nodeMemFrac & Fraction of Node memory for Jobs & 0.80 \\
 shuffleBufferFrac & Memory Fraction for shuffle & 0.25 \\
 readBufferFrac & Memory Fraction for reading & 0.25 \\
 \hline
\end{tabular}
\end{center}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\RETURN $\mathcal{P}_{new}, expectedResUsage$
	\end{algorithmic}
\end{algorithm}
...
\begin{figure}[h]
	\centering\includegraphics[height=4cm]{chart.png}
	%\vspace*{-15pt}
...
\begin{figure}[h]
	\centering\includegraphics[height=4cm]{chart_spark.png}
	%\vspace*{-15pt}
